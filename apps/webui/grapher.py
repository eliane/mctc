...
#Getters and Setters
	...
	def set_xaxis_min_ticksize(self, ticksize):
		 self.xaxis_min_ticksize = ticksize

	def get_xaxis_min_ticksize(self):
		 return self.xaxis_min_ticksize
...
#Methods that create the javascript string
	...
	def get_heading_string(self):
		string = "<h2>"+self.display_title+"</h2> \n"
		string += "<div> \n"
		...
		if(self.get_zoomable() == self.enable_zoom):
			string += "<div id=\""+self.get_plot_div()+"_miniature\" style=\"width:200px; height:120px;position:relative;top:-"+str(self.get_height())+"px;left:"+str(self.get_width())+"px;margin-left:20px;margin-top:50px\"> \n"
			string += "\t<div id=\""+self.get_plot_div()+"_overview\" style=\"width:166px;height:100px\"></div> \n"
			string += "\t<div id=\""+self.get_plot_div()+"_overviewLegend\" style=\"width:5px;height:10px\"></div>\n</div> \n"
		if (self.get_key_position() == self.key_outside and not(self.get_zoomable() == self.enable_zoom)):
			string += "\t<div id=\""+self.get_plot_div()+"_overviewLegend\" style=\"width:15px;height:10px\"></div>\n"
		string += "<script id=\"source\" language=\"javascript\" type=\"text/javascript\">\n\
		$(function(){\n"
		return string;
	...
	def get_axis_options(self):
		string = "\n\
				xaxis: {\n\
					...
					minTickSize: "+self.get_xaxis_min_ticksize()+",\n\
				},"
		string += "\n\
				yaxis: {\n\
    ...
    $(\"#" + self.get_plot_div() + "\").bind(\"plothover\", function (event, pos, item) {\n\
		...
		if(self.get_xaxis_mode() == "\"time\""):
			string += "var d = new Date();\n d.setTime(item.datapoint[0]);\n var mon = d.getUTCMonth() + 1;\n\
var day = d.getUTCDate();\n var year = d.getUTCFullYear();\n var show = day+\"/\"+mon+\"/\"+year;"
			string += "showTooltip(item.pageX, item.pageY, item.series.label + \": (\" + show + \", \" + y+ \")\");"
		else:
			string +="showTooltip(item.pageX, item.pageY, item.series.label + \": (\" + x + \", \" + y+ \")\");"
		string +="\n\
                }\n\
            }\n\
            else {\n\
...
plotOver();\n"
	...
	def get_zoom_with_overview(self):
        ...
        shadowSize: 0,\n\
        xaxis: { mode: "+ self.get_xaxis_mode()+", timeformat: \""+ self.time_format +"\", ticks: 4, min: "+self.get_xaxis_min()+", max: "+self.get_xaxis_max()+",},\n\
        yaxis: { mode: "+ self.get_yaxis_mode()+", timeformat: \""+ self.time_format +"\", ticks: 3, min: "+self.get_yaxis_min()+", max: "+self.get_yaxis_max()+",},\n\
